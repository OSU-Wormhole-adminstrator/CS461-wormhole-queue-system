...
def test_pastticket_resolution(test_client):
    ...
    t = Ticket(student_name="Old", table="T1", physics_course="211", status="live")
    db.session.add(t)
    db.session.commit()
...
def test_pastticket_forbidden_for_other_user(test_client):
    ...
    t = Ticket(
        student_name="Student", table="T2", physics_course="212", status="live"
    )
    db.session.add(t)
    db.session.commit()
...
def test_pastticket_admin_can_access_any_user(test_client):
    ...
    t = Ticket(student_name="Old", table="T3", physics_course="213", status="live")
    db.session.add(t)
    db.session.commit()
...
def test_livequeuetickets_includes_in_progress_in_order(test_client):
    ...
    t1 = Ticket(
        student_name="First", table="T1", physics_course="211", status="live"
    )
    t2 = Ticket(
        student_name="Second",
        table="T2",
    ...
    t3 = Ticket(
        student_name="Third", table="T3", physics_course="213", status="live"
    )
    db.session.add_all([t1, t2, t3])
    db.session.commit()
